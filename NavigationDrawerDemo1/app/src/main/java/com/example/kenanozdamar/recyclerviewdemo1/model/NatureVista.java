...
/**
 * Created by kenanozdamar on 7/9/17.
 */

public class NatureVista {
    private int imageID;
    ...
    public static ArrayList<NatureVista> getData(){
        ArrayList<NatureVista> dataList = new ArrayList<>();

        int[] images = getImages();

        for (int i = 0 ; i < images.length; i++) {
            NatureVista natureVista = new NatureVista();
            natureVista.setImageID(images[i]);
            natureVista.setTitle("Nature vista" + i);

            dataList.add(natureVista);
        }
